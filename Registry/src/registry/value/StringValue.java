 ...
 * @author 
 */
public class StringValue extends RegistryValue {

    protected String value;

    public StringValue(VkCell vkCell) {
        super(vkCell);
    }

    @Override
    protected void loadData(DataCell dataCell) throws RegistryException {
        ...
        for (int i = 0; i < (len / 2); i++) {
            if (dataCell.getBuffer().hasRemaining()) {
                ch = (char) dataCell.getBuffer().getChar();
                if (ch != 0) {
                    sb.append(ch);
                } else {
                    break;
                ...
                System.out.println("Buffer exhausted when processing string cell @" + dataCell.getOffset()
